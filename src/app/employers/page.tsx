    ...
    <motion.nav initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: smooth }} style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(10,10,10,0.9)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: isMobile ? '16px 20px' : '18px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'white', textDecoration: 'none' }}>
          ...
          <span style={{ fontSize: 12, color: '#71717a', marginLeft: 4 }}>for Employers</span>
        </Link>
        
        {isMobile ? (
          <>
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer', padding: 8 }} aria-label="Toggle menu">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
            ...
            <motion.a href="#contact" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#ff6b6b', color: 'white', fontSize: 14, fontWeight: 500, padding: '10px 20px', borderRadius: 100, textDecoration: 'none' }}>Get Started</motion.a>
          </div>
        )}
      </div>
    </motion.nav>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 48 : 80, alignItems: 'center' }}>
          <div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.4 }} style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              <motion.a href="#candidates" whileHover={{ scale: 1.03, y: -2 }} whileTap={{ scale: 0.98 }} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#ff6b6b', color: 'white', fontSize: 15, fontWeight: 600, padding: '14px 28px', borderRadius: 100, textDecoration: 'none', boxShadow: '0 8px 30px -5px rgba(255,107,107,0.4)' }}>
                Browse Candidates <ArrowRight style={{ width: 16, height: 16 }} />
              </motion.a>
              <motion.a href="#how-it-works" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', fontSize: 15, fontWeight: 500, padding: '14px 28px', borderRadius: 100, textDecoration: 'none' }}>
                See How It Works
              </motion.a>
            </motion.div>
          </div>
          ...
          <motion.div initial={{ opacity: 0, x: 40, scale: 0.95 }} animate={isInView ? { opacity: 1, x: 0, scale: 1 } : {}} transition={{ duration: 0.8, delay: 0.4 }}>
            <div style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.08) 0%, rgba(255,255,255,0.02) 100%)', borderRadius: 24, border: '1px solid rgba(255,255,255,0.1)', overflow: 'hidden' }}>
              <div style={{ padding: isMobile ? 16 : 24, borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <Search style={{ width: 18, height: 18, color: '#71717a' }} />
                  <span style={{ color: '#71717a', fontSize: 14 }}>Data Analytics candidates</span>
                </div>
                  ...
                  <motion.div key={c.name} initial={{ opacity: 0, x: 20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.6 + i * 0.1 }} whileHover={{ x: 4, background: 'rgba(255,255,255,0.08)' }} style={{ display: 'flex', alignItems: 'center', gap: 14, background: 'rgba(255,255,255,0.03)', borderRadius: 14, padding: isMobile ? 12 : 16, cursor: 'pointer', transition: 'all 0.2s ease' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 6, background: `${c.color}20`, color: c.color }}>{c.rank}</span>
                      </div>
                      <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                        {c.skills.map(s => (
                          <span key={s} style={{ fontSize: 11, color: '#71717a', background: 'rgba(255,255,255,0.05)', padding: '2px 8px', borderRadius: 4 }}>{s}</span>
                        ))}
                      </div>
                    </div>
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.05)' }}>
      <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: 8 }}>
        <BarChart3 style={{ width: 14, height: 14, color: '#ff6b6b' }} />
          ...
          {chartData.map((item, i) => {
            ...
            return (
              <div key={item.label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'flex-end' }}>
                ...
                />
                <span style={{ fontSize: 10, color: '#71717a', marginTop: 8, textAlign: 'center' }}>{item.label}</span>
              </div>
            );
          })}
        </div>
    ...
    <section id="candidates" ref={ref} style={{ background: '#0a0a0a', padding: isMobile ? '60px 20px' : '100px 24px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative' }}>
          ...
          <div style={{ padding: isMobile ? 20 : 32 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 24 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                <motion.div whileHover={{ scale: 1.05 }} style={{ width: 80, height: 80, borderRadius: 20, background: `linear-gradient(135deg, ${candidate.color}, ${candidate.color}88)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 28, fontWeight: 700 }}>{candidate.initials}</motion.div>
                <div>
                  <h3 style={{ fontSize: isMobile ? 24 : 32, fontWeight: 800, color: 'white', marginBottom: 8 }}>{candidate.name}</h3>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, fontSize: 14, color: '#a1a1aa' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Briefcase style={{ width: 14, height: 14 }} />{candidate.track}</span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><MapPin style={{ width: 14, height: 14 }} />{candidate.location}</span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Clock style={{ width: 14, height: 14 }} />{candidate.availability}</span>
                  </div>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 40, fontWeight: 900, color: 'white' }}>{candidate.score}</div>
                  <div style={{ fontSize: 12, color: '#71717a' }}>Overall Score</div>
                </div>
                <div style={{ background: candidate.color, color: 'white', fontSize: 14, fontWeight: 700, padding: '12px 24px', borderRadius: 100 }}>{candidate.rank}</div>
              </div>
            </div>
          </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                    {[
                      ...
                      <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 16, padding: 20, border: '1px solid rgba(255,255,255,0.05)', textAlign: 'center' }}>
                  ...
                  {/* Skills */}
                  <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 20, padding: 24, border: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    {challenges.map((ch, i) => (
                      <motion.div key={ch.title} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + i * 0.1 }} whileHover={{ background: 'rgba(255,255,255,0.05)' }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.02)', borderRadius: 12, padding: 16, cursor: 'pointer', transition: 'all 0.2s ease' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 6, background: '#ff6b6b20', color: '#ff6b6b' }}>{ch.rank}</span>
                        </div>
                      </motion.div>
                    ))}
                  </div>
            ...
            <motion.div key="challenge" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 20, padding: isMobile ? 20 : 28, border: '1px solid rgba(255,255,255,0.05)', marginBottom: 20 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 24 }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                      <span style={{ background: '#ff6b6b20', color: '#ff6b6b', fontSize: 12, fontWeight: 600, padding: '4px 10px', borderRadius: 6 }}>Rushly</span>
                      <span style={{ background: '#10b98120', color: '#10b981', fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 6 }}>Top 1%</span>
                      <span style={{ color: '#52525b', fontSize: 12 }}>2 weeks</span>
                    </div>
                    <h3 style={{ fontSize: 22, fontWeight: 700, color: 'white', marginBottom: 8 }}>Customer Churn Prediction</h3>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {['SQL', 'Python', 'Tableau'].map(t => (
                      <span key={t} style={{ background: 'rgba(255,255,255,0.05)', color: '#a1a1aa', fontSize: 12, padding: '6px 12px', borderRadius: 6 }}>{t}</span>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(4, 1fr)', gap: 16 }}>
                  {[
                    { label: 'At-Risk Revenue Identified', value: '$2.3M', sub: '+34% improvement' },
                    { label: 'Model Accuracy', value: '89%', sub: 'F1 Score' },
                    { label: 'Predicted Churn Reduction', value: '23%', sub: 'Over 6 months' },
                    { label: 'Recommendations', value: '12', sub: 'Actionable items' },
                  ].map((m, i) => (
                    <motion.div key={m.label} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 + i * 0.1 }} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 16, textAlign: 'center' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: 20, marginBottom: 20 }}>
                ...
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <Database style={{ width: 14, height: 14, color: '#8b5cf6' }} />
                      <span style={{ fontSize: 12, color: '#a1a1aa' }}>churn_analysis.sql</span>
                    </div>
                    <span style={{ fontSize: 10, color: '#52525b' }}>247 lines</span>
                  </div>
                  <div style={{ padding: 16, fontFamily: 'monospace', fontSize: 11, color: '#a1a1aa', lineHeight: 1.7, maxHeight: 200, overflow: 'hidden' }}>
                    ...
                    <div style={{ paddingLeft: 24 }}><span style={{ color: '#f59e0b' }}>DATEDIFF</span>(day, c.signup_date, <span style={{ color: '#f59e0b' }}>GETDATE</span>()) <span style={{ color: '#8b5cf6' }}>AS</span> tenure_days,</div>
                    <div style={{ paddingLeft: 24 }}><span style={{ color: '#f59e0b' }}>COUNT</span>(<span style={{ color: '#8b5cf6' }}>DISTINCT</span> o.order_id) <span style={{ color: '#8b5cf6' }}>AS</span> total_orders,</div>
                    <div style={{ paddingLeft: 24 }}><span style={{ color: '#f59e0b' }}>AVG</span>(o.order_value) <span style={{ color: '#8b5cf6' }}>AS</span> avg_order_value,</div>
                    <div style={{ paddingLeft: 24 }}><span style={{ color: '#f59e0b' }}>MAX</span>(o.order_date) <span style={{ color: '#8b5cf6' }}>AS</span> last_order_date</div>
                    <div style={{ paddingLeft: 16 }}><span style={{ color: '#8b5cf6' }}>FROM</span> customers c</div>
                    <div style={{ paddingLeft: 16 }}><span style={{ color: '#8b5cf6' }}>LEFT JOIN</span> orders o <span style={{ color: '#8b5cf6' }}>ON</span> c.customer_id = o.customer_id</div>
                ...
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.35 }} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <Code style={{ width: 14, height: 14, color: '#10b981' }} />
                      <span style={{ fontSize: 12, color: '#a1a1aa' }}>churn_model.py</span>
                    </div>
                    <span style={{ fontSize: 10, color: '#52525b' }}>189 lines</span>
                  </div>
                  <div style={{ padding: 16, fontFamily: 'monospace', fontSize: 11, color: '#a1a1aa', lineHeight: 1.7, maxHeight: 200, overflow: 'hidden' }}>
                    <div><span style={{ color: '#8b5cf6' }}>import</span> pandas <span style={{ color: '#8b5cf6' }}>as</span> pd</div>
                    <div><span style={{ color: '#8b5cf6' }}>from</span> sklearn.ensemble <span style={{ color: '#8b5cf6' }}>import</span> RandomForestClassifier</div>
                    <div><span style={{ color: '#8b5cf6' }}>from</span> sklearn.model_selection <span style={{ color: '#8b5cf6' }}>import</span> train_test_split</div>
                    <div style={{ marginTop: 8 }}></div>
                    <div><span style={{ color: '#8b5cf6' }}>def</span> <span style={{ color: '#f59e0b' }}>train_churn_model</span>(df):</div>
                    <div style={{ paddingLeft: 16 }}><span style={{ color: '#52525b' }}># Feature engineering</span></div>
                    <div style={{ paddingLeft: 16 }}>features = [<span style={{ color: '#10b981' }}>'tenure_days'</span>, <span style={{ color: '#10b981' }}>'total_orders'</span>,</div>
                    <div style={{ paddingLeft: 48 }}><span style={{ color: '#10b981' }}>'avg_order_value'</span>, <span style={{ color: '#10b981' }}>'days_since_last'</span>]</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: 20, marginBottom: 20 }}>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <TrendingUp style={{ width: 14, height: 14, color: '#8b5cf6' }} />
                    ...
                    {[
                      ...
                      <motion.div key={f.name} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 + i * 0.05 }} style={{ marginBottom: i < 4 ? 12 : 0 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                          <span style={{ fontSize: 12, color: '#a1a1aa' }}>{f.name}</span>
                          <span style={{ fontSize: 12, color: 'white', fontWeight: 600 }}>{(f.value * 100).toFixed(0)}%</span>
                        </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 20 }}>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 16, padding: 24, border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <motion.div whileHover={{ scale: 1.05 }} style={{ width: 64, height: 64, background: '#ff6b6b20', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                      <Play style={{ width: 28, height: 28, color: '#ff6b6b', marginLeft: 2 }} />
                    </motion.div>
                    <div>
                      <div style={{ fontSize: 16, fontWeight: 600, color: 'white', marginBottom: 4 }}>Live Defense Recording</div>
                      <div style={{ fontSize: 13, color: '#71717a' }}>45 min • Watch Sarah present and answer panel questions</div>
                      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                        <span style={{ fontSize: 11, color: '#10b981', background: '#10b98120', padding: '2px 8px', borderRadius: 4 }}>HD Quality</span>
                        <span style={{ fontSize: 11, color: '#8b5cf6', background: '#8b5cf620', padding: '2px 8px', borderRadius: 4 }}>Timestamped</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
                
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 16, padding: 24, border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <motion.div whileHover={{ scale: 1.05 }} style={{ width: 64, height: 64, background: '#10b98120', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                      <PieChart style={{ width: 28, height: 28, color: '#10b981' }} />
                    </motion.div>
                    <div>
                      <div style={{ fontSize: 16, fontWeight: 600, color: 'white', marginBottom: 4 }}>Interactive Dashboard</div>
                      <div style={{ fontSize: 13, color: '#71717a' }}>Tableau • Explore the churn analysis dashboard</div>
                      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                        <span style={{ fontSize: 11, color: '#f59e0b', background: '#f59e0b20', padding: '2px 8px', borderRadius: 4 }}>12 Views</span>
                        <span style={{ fontSize: 11, color: '#ff6b6b', background: '#ff6b6b20', padding: '2px 8px', borderRadius: 4 }}>Filters Enabled</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
            ...
            <motion.div key="evaluation" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 20, marginBottom: 24 }}>
                {[
                  ...
                  <motion.div key={item.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 20, padding: 28, border: '1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                      ...
                      <span style={{ fontSize: 32, fontWeight: 900, color: item.color }}>{item.score}</span>
                    </div>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 20, padding: isMobile ? 20 : 28, border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {evaluation.comments.map((c, i) => (
                    <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.1 }} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 16, padding: 20 }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ delay: 0.6 }} style={{ marginTop: 24, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} style={{ flex: 1, minWidth: 150, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#ff6b6b', color: 'white', fontSize: 14, fontWeight: 600, padding: '16px 24px', borderRadius: 12, border: 'none', cursor: 'pointer' }}>
            <MessageSquare style={{ width: 16, height: 16 }} /> Contact Sarah
          </motion.button>
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} style={{ flex: 1, minWidth: 150, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', fontSize: 14, fontWeight: 600, padding: '16px 24px', borderRadius: 12, cursor: 'pointer' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 20, alignItems: 'stretch' }}>
          {plans.map((plan, i) => (
            <motion.div key={plan.name} initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 + i * 0.1 }} whileHover={{ y: -8 }} style={{ background: plan.popular ? '#0a0a0a' : 'white', borderRadius: 24, padding: isMobile ? 24 : 32, border: plan.popular ? '2px solid #ff6b6b' : '1px solid #e5e5e5', position: 'relative', display: 'flex', flexDirection: 'column' }}>
              {plan.popular && (
                <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', background: '#ff6b6b', color: 'white', fontSize: 11, fontWeight: 700, padding: '6px 16px', borderRadius: 100, textTransform: 'uppercase' }}>Most Popular</div>
              )}
              <div style={{ marginBottom: 24 }}>
                <h3 style={{ fontSize: 20, fontWeight: 700, color: plan.popular ? 'white' : '#0a0a0a', marginBottom: 8 }}>{plan.name}</h3>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                  <span style={{ fontSize: 40, fontWeight: 900, color: plan.popular ? 'white' : '#0a0a0a' }}>{plan.price}</span>
