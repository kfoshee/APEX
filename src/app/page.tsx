...
function usePremiumScroll() {
  useEffect(() => {
    ...
    // Skip on touch devices — native momentum scrolling is already premium
    ...
    let running = false;
    const ease = 0.088; // lower = smoother glide
    ...
    const onWheel = (e: WheelEvent) => {
      e.preventDefault();
      // Normalize deltaY across browsers & modes (pixel vs line vs page)
      let delta = e.deltaY;
      if (e.deltaMode === 1) delta *= 36;   // line mode
    ...
    /* anchor-link clicks: animate to target instead of jumping */
    ...
    /* sync after any programmatic scroll or browser back/forward */
    const onScroll = () => {
      if (!running) {
      ...
      <div style={{ padding: isMobile ? '60px 20px' : '80px 24px', maxWidth: 1200, margin: '0 auto' }}>
        ...
        <motion.div ref={ctaRef} initial={{ opacity: 0, y: 60, scale: 0.95 }} animate={ctaInView ? { opacity: 1, y: 0, scale: 1 } : {}} transition={{ duration: 0.8, ease: smooth }} style={{ background: '#0a0a0a', borderRadius: isMobile ? 16 : 24, padding: isMobile ? 24 : 48, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'stretch', gap: 0, position: 'relative', flexDirection: isMobile ? 'column' : 'row' }}>
            <motion.div initial={{ opacity: 0, y: 30 }} animate={ctaInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.3 }} style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: isMobile ? '20px 16px' : '28px 24px', display: 'flex', gap: 16, position: 'relative' }}><div style={{ position: 'absolute', top: -12, left: 20, background: '#10b981', color: 'white', width: 24, height: 24, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800 }}>1</div><div style={{ width: 44, height: 44, background: '#10b981', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 21V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v16" /></svg></div><div style={{ minWidth: 0 }}><div style={{ fontSize: isMobile ? 16 : 18, fontWeight: 700, color: 'white', marginBottom: 6 }}>Get recruited early</div><div style={{ fontSize: isMobile ? 13 : 15, color: '#a1a1aa', lineHeight: 1.5 }}>Employers browse portfolios and reach out. Some get hired before finishing.</div></div></motion.div>
            ...
            <AnimatePresence>
              {showPrompts && isPlaying && (
                <motion.div
                  ...
                  style={{ position: 'absolute', top: 24, left: 0, right: 0, display: 'flex', justifyContent: 'center', gap: 12, zIndex: 20, pointerEvents: 'none', padding: '0 12px' }}
                >
                  {isMuted && (
                    <motion.button
  ...
  return (
    <section ref={ref} style={{ padding: isMobile ? '60px 0' : '120px 0', background: '#0a0a0a', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: isMobile ? '0 20px' : '0 24px', position: 'relative' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: 14 }}>{tracks.map((track, i) => (<motion.div key={track.name} initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, ease: smooth, delay: 0.2 + i * 0.08 }} whileHover={track.unlocked ? { y: -4, scale: 1.01 } : {}} style={{ position: 'relative', background: track.unlocked ? 'linear-gradient(135deg, rgba(255,107,107,0.1) 0%, rgba(20,20,23,1) 100%)' : '#111113', borderRadius: 18, padding: isMobile ? 18 : 24, border: track.unlocked ? '1px solid rgba(255,107,107,0.3)' : '1px solid #1f1f23', cursor: track.unlocked ? 'pointer' : 'default', transition: 'all 0.3s ease' }}><div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}><div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, minWidth: 0 }}><div style={{ width: 44, height: 44, borderRadius: 12, background: track.unlocked ? 'linear-gradient(135deg, #ff6b6b, #ff5252)' : '#1f1f23', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><track.Icon style={{ width: 22, height: 22, color: track.unlocked ? 'white' : '#52525b' }} /></div><div style={{ minWidth: 0 }}><h3 style={{ fontSize: isMobile ? 16 : 18, fontWeight: 700, color: track.unlocked ? 'white' : '#52525b', marginBottom: 4 }}>{track.name}</h3><p style={{ fontSize: isMobile ? 13 : 14, color: track.unlocked ? '#a1a1aa' : '#3f3f46' }}>{track.desc}</p></div></div>{track.unlocked ? <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: '#ff6b6b', color: 'white', fontSize: 11, fontWeight: 700, padding: '5px 10px', borderRadius: 100, textTransform: 'uppercase', letterSpacing: '0.05em', flexShrink: 0, whiteSpace: 'nowrap' }}><motion.div style={{ width: 5, height: 5, background: 'white', borderRadius: '50%' }} animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 2, repeat: Infinity }} />Open</div> : <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'transparent', border: '1px solid #27272a', color: '#52525b', fontSize: 11, fontWeight: 500, padding: '5px 10px', borderRadius: 100, flexShrink: 0, whiteSpace: 'nowrap' }}><Lock style={{ width: 11, height: 11 }} />Soon</div>}</div>{track.unlocked && <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 0.5, delay: 0.5 }} style={{ marginTop: 18, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.1)' }}><motion.button onClick={() => navigateWithScroll(APPLY_URL)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: '#ff6b6b', fontSize: 14, fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }} whileHover={{ x: 4 }}>Start this track<ArrowRight style={{ width: 14, height: 14 }} /></motion.button></motion.div>}</motion.div>))}</div>
...
export default function Page() {
  ...
  return (
    <>
      ...
      `}</style>
      <main style={{ minHeight: '100vh', overflowX: 'hidden' }}><Hero /><Journey /><WhatIsApex /><Tracks /><Comparison /><Pricing /><Employers /><FAQ /><Footer /></main>
    </>
  );
}